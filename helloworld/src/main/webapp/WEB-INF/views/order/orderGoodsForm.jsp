...
<c:if test="${flagAirStay != 'airRnM'}">
    <c:set var="total_order_priceGo" value="${airEachDataMap.economyCharge* airEachDataMap.qty}" /> <!-- 왕복다구간 1 금액 -->
    <c:set var="total_order_priceBack" value="${airEachDataMap.economyCharge* airEachDataMap.qty}" /> <!-- 왕복다구간 2 금액 -->
</c:if>
...
<form  name="form_order">	
				...
				<tr>
					...
					<td rowspan="2">
						<c:if test="${airEachDataMap.prestigeCharge eq '0' }">
							...
							<c:set var="total_order_priceGo" 	value="${airEachDataMap.economyCharge }" />
						...
						<c:if test="${airEachDataMap.prestigeCharge ne '0' }">
							...
							<input id="economyBtn" type="button" value="일반석 선택" onclick="flagAirStay = 'air'; select_economy()"  class="subm" style="width: 120px;"/>
							<div id="brDiv"></div>
							<input id="businessBtn" type="button" value="비즈니스석 선택" onclick="flagAirStay = 'air'; select_business()"  class="subm" style="width: 120px;"/>
								<c:set var="total_order_priceGo" 	value="${airEachDataMap.economyCharge }" />
						</c:if>
					</td>
	...
	<c:if test="${flagAirStay eq 'airRnM' }">
		...
		<input id="GOValueRealTime"  value="${total_order_priceGo}" /><!--  type="hidden" -->
		<input id="BACKValueRealTime" value="${total_order_priceBack}" />
		<input id="TOTALValueRealTime" value="${total_order_price}" />
		<div class="clear"></div><br>
	</c:if>
	<!-- =======================================항공 왕복/다구간================================================= -->
	<!-- =======================================숙박========================================================== -->
	<c:if test="${flagAirStay eq 'stay' }">
		...
		<c:choose>
			<c:when test="${not empty goodsStayVOEach.goods_stay_sales_price and goodsStayVOEach.goods_stay_sales_price != 0}">
				<c:set var="total_order_price" value="${goodsStayVOEach.goods_stay_sales_price* stayDays}" />
			</c:when>
			<c:otherwise>
				<c:set var="total_order_price" value="${goodsStayVOEach.goods_stay_price* stayDays}" />
			</c:otherwise>
		</c:choose>
 		<div class="clear"></div><br>
	</c:if>
